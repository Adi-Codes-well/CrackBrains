  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const productsRes = await axios.get(`${import.meta.env.VITE_APP_API_URL}/api/products/myproducts`, {
          headers: { Authorization: token },
        });
        setProducts(productsRes.data);

        // 3. Fetch supplier's orders
        const ordersRes = await axios.get(`${import.meta.env.VITE_APP_API_URL}/api/orders/supplier`, {
          headers: { Authorization: token },
        });
        setOrders(ordersRes.data);

        // 4. Fetch trust score for the current supplier
        const trustRes = await axios.get(`${import.meta.env.VITE_APP_API_URL}/api/trust/${currentSupplierId}`, {
            headers: { Authorization: token },
        });
